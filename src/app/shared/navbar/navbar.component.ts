...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  // Propiedades para navegación por scroll
  ...
  ngOnInit(): void {
    ...
    // Suscribirse al ScrollService
    if (this.isBrowser) {
      const scrollSub = this.scrollService.scrollPosition$.subscribe(scrollPosition => {
        this.isScrolled = scrollPosition > 50;
      });

      const activeSectionSub = this.scrollService.activeSection$.subscribe(activeSection => {
        this.activeSection = activeSection;
      });

      this.subscriptions.push(scrollSub, activeSectionSub);
    }
  }
  ...
  // Escuchamos el scroll de la ventana para efectos de transparencia
  @HostListener('window:scroll', ['$event'])
  onWindowScroll(): void {
    if (this.isBrowser) {
      const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
      this.isScrolled = scrollTop > 50; // Cambia el estado cuando se hace scroll más de 50px
    }
  }
  ...
  // === MÉTODOS PARA NAVEGACIÓN POR SCROLL ===

  // Inicializar Intersection Observer para detectar secciones activas
  private initializeIntersectionObserver(): void {
    if (!this.isBrowser) return;

    const options = {
      root: null,
      rootMargin: '-20% 0px -20% 0px', // Detectar cuando una sección está en el centro de la pantalla
      threshold: 0.3
    };
    ...
    // Observar todas las secciones de la landing page
    setTimeout(() => { // Usar setTimeout para asegurar que los elementos existan
      this.landingSections.forEach(section => {
  ...
  // Método para hacer scroll suave a una sección específica usando ScrollService
  scrollToSection(sectionId: string): void {
    if (!this.isBrowser) return;

    // Usar el ScrollService para navegación suave
    this.scrollService.scrollToSection(sectionId, 80);

    // Cerrar menús móviles después del scroll
    this.mobileMenuOpen = false;
    this.mobileSubmenuOpen = false;
    this.desktopSubmenuOpen = false;
  }
  ...
  // Método para scroll suave hacia arriba usando ScrollService
  scrollToTop(): void {
    if (!this.isBrowser) return;
